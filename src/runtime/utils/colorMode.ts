...
export const buildColorModeInitScript = (): string => {
	...
	if (DANGEROUS_TOKEN.test(script)) {
		throw new Error(
			'nuxt-theme-transitions: generated init script contains the literal text "typeof window", which Nitro\'s server bundler textually replaces with "undefined" and corrupts this script. Use `typeof globalThis.window`, `typeof matchMedia`, or `typeof localStorage` instead in whichever function introduced it.',
		);
	}
